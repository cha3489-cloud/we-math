...
create policy "roles visible to self or admin" on public.user_roles for select to authenticated using (user_id = auth.uid() or public.is_admin());
...
create policy "students read assigned work" on public.assignments for select to authenticated using (student_id = auth.uid() or public.is_admin());
...
begin
 if auth.uid() is null or new.student_id <> auth.uid() then raise exception 'invalid student'; end if;
...
create policy "students read own submissions" on public.submissions for select to authenticated using (student_id = auth.uid() or public.is_admin());
create policy "students submit assigned work" on public.submissions for insert to authenticated with check (student_id = auth.uid() and status = 'submitted');
...
create policy "students read own feedback" on public.feedback for select to authenticated using (public.is_admin() or exists (select 1 from public.submissions s where s.id = submission_id and s.student_id = auth.uid()));
...
create policy "assignment files readable by assignee" on storage.objects for select to authenticated using (bucket_id = 'assignment-files' and (public.is_admin() or exists (select 1 from public.assignments a where a.student_id = auth.uid() and name = any(a.attachment_paths))));
...
create policy "submission files readable by owner" on storage.objects for select to authenticated using (bucket_id = 'submission-files' and ((storage.foldername(name))[1] = auth.uid()::text or public.is_admin()));
create policy "students upload submission files" on storage.objects for insert to authenticated with check (bucket_id = 'submission-files' and (storage.foldername(name))[1] = auth.uid()::text and exists (select 1 from public.assignments a where a.id::text = (storage.foldername(name))[2] and a.student_id = auth.uid()));
create policy "students delete unsubmitted files" on storage.objects for delete to authenticated using (bucket_id = 'submission-files' and (storage.foldername(name))[1] = auth.uid()::text and not exists (select 1 from public.submissions s where name = any(s.file_paths)));
