...
alter table public.profiles add column if not exists pin_generation bigint not null default 0;
...
create or replace function public.is_not_suspended() returns boolean language sql stable security definer set search_path = public as $$ select exists (select 1 from public.profiles where id = auth.uid() and suspended_at is null) $$;
...
create or replace function public.is_active_user() returns boolean language sql stable security definer set search_path = public as $$ select exists (select 1 from public.profiles where id = auth.uid() and suspended_at is null and must_change_pin = false) $$;
revoke all on function public.is_active_user() from public, anon;
grant execute on function public.is_active_user() to authenticated;
create or replace function public.is_admin() returns boolean language sql stable security definer set search_path = public as $$ select exists (select 1 from public.user_roles r join public.profiles p on p.id = r.user_id where r.user_id = auth.uid() and r.role = 'admin' and p.suspended_at is null and p.must_change_pin = false) $$;
revoke all on function public.is_admin() from public, anon;
grant execute on function public.is_admin() to authenticated;
create or replace function public.mark_pin_reset(p_user_id uuid) returns void language plpgsql security definer set search_path = public as $$
begin
 update public.profiles set must_change_pin = true, pin_generation = pin_generation + 1 where id = p_user_id;
 if not found then raise exception 'profile not found'; end if;
end;
$$;
revoke all on function public.mark_pin_reset(uuid) from public, anon, authenticated;
grant execute on function public.mark_pin_reset(uuid) to service_role;
create policy "roles visible to self or admin" on public.user_roles for select to authenticated using (public.is_not_suspended() and (user_id = auth.uid() or public.is_admin()));
drop policy if exists "본인 조회" on public.profiles;
drop policy if exists "본인 수정" on public.profiles;
create policy "non-suspended users read own profile" on public.profiles for select to authenticated using (id = auth.uid() and public.is_not_suspended());
...
begin
 ...
 if cardinality(new.file_paths) > 3 then raise exception 'too many submission files'; end if;
 if exists (
   select 1 from unnest(new.file_paths) as path
   where path !~ ('^' || auth.uid()::text || '/' || new.assignment_id::text || '/[^/]+$')
      or not exists (select 1 from storage.objects o where o.bucket_id = 'submission-files' and o.name = path)
 ) then raise exception 'invalid submission file'; end if;
...
grant usage on type public.app_role, public.submission_status to authenticated;
...
insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types) values ('assignment-files', 'assignment-files', false, 10485760, array['application/pdf', 'image/jpeg', 'image/png', 'image/webp']), ('submission-files', 'submission-files', false, 10485760, array['application/pdf', 'image/jpeg', 'image/png', 'image/webp']) on conflict (id) do update set public = false, file_size_limit = excluded.file_size_limit, allowed_mime_types = excluded.allowed_mime_types;
